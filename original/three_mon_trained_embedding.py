...
Training:
    1) Read csi_data and, 2)Learning the embedding through CNN and 3) ProtoNet
Testing:
...
result_out_name = 'results/result_A1A2_' + data_folder + '.pt'
#%%
...
class ProtoNet(nn.Module):
  ...
  def set_forward_loss(self, sample):
    ...
    """
    sample_images = sample['csi_mats'].cuda(1)
    n_way = sample['n_way']
    ...
    target_inds = target_inds.cuda(1)
...
def test(model1, model2, model3, test_x_1, test_y_1, test_x_2, test_y_2, test_x_3, test_y_3, n_way, n_support, n_query, test_episode):
  ...
  for episode in tnrange(test_episode):
    ...
    out = torch.cat([output_1['y_hat'].view(n_way*n_query,1),
                       output_2['y_hat'].view(n_way*n_query,1), output_3['y_hat'].view(n_way*n_query,1)],1).cpu()
    true_out = torch.zeros(n_way*n_query,1)
    for i in range(n_way*n_query):
        u, count = torch.unique(out[i,:], return_counts = True)
        true_out[i] = u[torch.argmax(count)]
    a = true_out.reshape(n_way,n_query).int()
    for cls in range(n_way):
        conf_mat[cls,:] = conf_mat[cls,:] + torch.bincount(a[cls,:], minlength = n_way) 
        
    acc = torch.eq(true_out, target_inds).float().mean()
    #running_loss += output['loss']
    running_acc += acc
